...
# number of requests sent this month
request_counter = 0
# maxmimum number of free requests
...
def get_response(waypoints_list):
    '''sends the POST request and returns the Json response'''
  
    global request_counter
    
    # removes first and last waypoint coordinates as they will be used as origin and destination
    ...
    # body of request - 
    # Origin and destination are start and end points of the request, intermediates are the waypoints used in between
    # traffic on polyline gives traffic conditions - NORMAL, SLOW and TRAFFIC JAM at parts on the polyline
    json_data = {
...
def get_roads_df(waypoints_df,response,usertime,poly_line):
    ...
    # Row will be in the following format:
    # +----+---------------------+-------------+--------------+--------------------+------------------------------------+-------------+-------------+-----------+-----------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+------------+------------+----------------------------------------+
    # |    | DateTime            | User Time   | Road         | Travel Direction   | Polyline                           |   start_lat |   start_lng |   end_lat |   end_lng | Intermediates                                                                                                                                                                      |   Distance | Duration   | Response                               |
    # +====+=====================+=============+==============+====================+====================================+=============+=============+===========+===========+====================================================================================================================================================================================+============+============+========================================+
    # |  0 | 07/10/2023 13:35:00 | 13:35:00    | Dakshin Marg | SE                 | mmmzDugssMmc@f`@oa@b]IWfa@}\vc@q`@ |     30.7121 |     76.7809 |   30.7121 |    76.781 | List of dictionaries of all waypoints used, containing their latitude, longitude, name and number of the junction that the side of the road the waypoints is on is heading towards |       4207 | 729s       | Json Response is stored in this column |
    # +----+---------------------+-------------+--------------+--------------------+------------------------------------+-------------+-------------+-----------+-----------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+------------+------------+----------------------------------------+  
    ...
    # stores actual time that the request was sent at
    response_df["DateTime"].loc[0] = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    # stores time define by user in the times list that the current request corresponds to
    ...
    # creating a list containing dictionaries for all waypoints used 
    # Each dictionary contains latitude, longtitude, junction name and number that the side of the road that the waypoints is on is heading towards
    ...
    # distance of the entire length of the road going both ways
    response_df["Distance"].loc[0] = data['routes'][0]['distanceMeters']
    # duration taken to traverse the entire road both ways
    response_df["Duration"].loc[0] = data['routes'][0]['duration']
    # json response recieved from the POST request
...
def get_error_roads_df(waypoints_df,response,usertime,poly_line):
    ...
    # Row is in the same format as the previous method, but has the following changes
    # Distance and Duration are stores as np.nan and None respectively
    # Response column stores the error response
...
def get_junctions_df(waypoints_df, response, usertime, poly_line):
    ...
    # Row is stored in the following format
    # +----+---------------------+-------------+-----------------+---------------+---------------+-------------------------------------------------------------+-------------+-------------+-----------+-----------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+------------+------------+----------------------------------------+
    # |    | DateTime            | User Time   | Junction Name   |   Junction no |   No of Turns | Polyline                                                    |   start_lat |   start_lng |   end_lat |   end_lng | Intermediates                                                                                                                                                                |   Distance | Duration   | Response                               |
    # +====+=====================+=============+=================+===============+===============+=============================================================+=============+=============+===========+===========+==============================================================================================================================================================================+============+============+========================================+
    # |  0 | 07/10/2023 13:35:01 | 13:35:00    | Labour Chowk    |            35 |             8 | {qnzDmfrsM_ZgBLM}Fx`@IWp]pASHhCy_@PPgC|^SHs\cAIWxFs_@LM~XbB |     30.7179 |     76.7756 |    30.718 |   76.7757 | List of dictionaries of all pairs of waypoints used for each turn. Each dictionary contains lat, long, travel direction and description of start and end points of each turn |      11270 | 2102s      | Json Response is stored in this column |
    # +----+---------------------+-------------+-----------------+---------------+---------------+-------------------------------------------------------------+-------------+-------------+-----------+-----------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+------------+------------+----------------------------------------+
    ...
    # creating a list containing dictionaries for all pairs waypoints used for each turn 
    # Each dictionary contains latitude, longtitude, direction and description of start and end point of each turn
    intermediates_list = []    
...
# groups of roads for each road request
waypoint_groups = waypoints.groupby("loc_road")      

# contains selected junctions
...
# dataframe containing all start and end points of each turn at a junction (including straights and U-Turns)
...
#retains only selected junctions
...
# Bearing is predefined as 45 for NE, 135 for SE, 225 for SW, 315 for NW (based on the angle they make with north)
# gives turn angle, which is used to figure out whether the pair of merge points is a straight, left, right or U turn
destinations["turn_angle"] = ((destinations["Bearing_to"]-destinations["Bearing_from"])+360)%360

# turn angles: 0 for straights, 90 for right turns, 180 for U-Turns, 270 for left turns  
directions_dict = {"turn_angle":[0,90,180,270], "Turn":["Straight", "Right", "U-turn", "Left"]}

# adds type of turn (straight, left, right or U) to destinations
...
# destinations is split into left and right turns
...
#travel_direction_from stores the direction of the road that the turn starts from
...
#joining lefts and rights df
destinations_right_left = pd.concat([destinations_right,destinations_left])

# groups of junctions to be iterated through for each request
...
while True:
    ...
    for road_name,roads_group_df in waypoint_groups:
        ...
        # stores list of waypoints to be used in the reuqest
        ...
        while True:
            try:
                ...
                # if there is an error in the response recieved from the API
                print("ERROR")
                #logging
    ...
    for junction_name,junctions_group_df in destination_groups:
        ...
        errors=0
        # stores list of waypoints to be used in the request
